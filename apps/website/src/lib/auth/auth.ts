...
if (!process.env.BETTER_AUTH_SECRET || !process.env.NEXT_PUBLIC_WEBSITE_URL) {
  const missingEnvVars = []
  if (!process.env.BETTER_AUTH_SECRET) missingEnvVars.push("BETTER_AUTH_SECRET")
  if (!process.env.NEXT_PUBLIC_WEBSITE_URL) missingEnvVars.push("NEXT_PUBLIC_WEBSITE_URL")
